...
프롬프트 전략은 요구사항을 한 번에 구현하지 않고, 기능 단위와 commit 단위로 나누어 진행하는 방식으로 사용했습니다. 각 단계마다 구현 목적, 수정 파일, 검증 명령, 예상 결과를 확인한 뒤 다음 단계로 넘어가도록 하여 제출 코드를 직접 설명할 수 있는 수준으로 검증했습니다.
